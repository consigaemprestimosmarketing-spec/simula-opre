...
export function adminCredentialsConfigured() {
  return adminConfiguration().password.length >= 8;
}
...
export function hasAllowedRequestOrigin(request) {
  ...
  try {
    const receivedHost = new URL(receivedOrigin).host.toLowerCase();
    ...
    const allowedHosts = new Set(
      [requestHost, hostHeader, ...forwardedHosts].filter(Boolean)
    );
    return allowedHosts.has(receivedHost);
  } catch {
    return false;
  }
}
...
<body>
  <main class='admin-shell'>
    <header class='admin-bar'>
      <a href='/' aria-label='Voltar ao simulador'>
...
<body class='admin-error-body'>
  <main class='admin-error'>
    <a href='/' aria-label='Voltar ao simulador'>
      <img src='/assets/logo-principal.png' alt='Consiga Empréstimos'>
    </a>
